...
export default function CoverLetter() {
  ...
  if (!email) {
    return (
      <div className="flex flex-col items-center justify-center h-64 space-y-4 text-center">
        <p className="text-foreground/50">Build your CV first so the AI has something real to write from.</p>
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div>
        <h1 className="text-3xl font-black text-foreground">Cover Letter Generator</h1>
        <p className="text-foreground/40 text-sm mt-1">AI writes a real letter from your actual CV — nothing pre-written or templated.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="space-y-4">
          <TiltCard className="glass rounded-2xl p-5 space-y-3" max={4}>
            <h3 className="text-foreground font-semibold text-sm">Job</h3>
            {tracked.length > 0 && (
              <select
          ...
          <TiltCard className="glass rounded-2xl p-5" max={4}>
            <h3 className="text-foreground font-semibold text-sm mb-3">Tone</h3>
              ...
              {tones.map((t) => (
                <button
                  ...
                  className={`w-full text-left p-3 rounded-xl border transition-all text-sm font-semibold ${
                    tone === t ? "bg-violet-500/20 border-violet-500/40 text-violet-700" : "glass text-foreground/50 hover:text-foreground/80"
                  }`}
                >
                  {t}
                </button>
        ...
        <div className="lg:col-span-2 glass rounded-2xl overflow-hidden flex flex-col">
          <div className="flex items-center justify-between p-4 border-b border-foreground/5">
            <div>
              <span className="text-foreground font-semibold text-sm">{company || "—"} {role && `— ${role}`}</span>
              <span className="ml-2 text-xs text-foreground/40">{tone}</span>
            </div>
            {letter && (
              <div className="flex gap-2">
                <button onClick={copy} className={`btn-glass px-3 py-1.5 rounded-xl text-xs font-semibold transition-all inline-flex items-center gap-1.5 ${copied ? "text-emerald-600" : "text-foreground/60"}`}>
                  {copied ? <><Check className="w-3.5 h-3.5" /> Copied!</> : "Copy"}
                </button>
                <button onClick={download} className="btn-glass px-3 py-1.5 rounded-xl text-xs font-semibold text-foreground/60 inline-flex items-center gap-1.5">
          ...
          {letter ? (
            <textarea
              className="flex-1 p-5 text-sm text-foreground/70 leading-relaxed resize-none outline-none"
              style={{ background: "transparent", minHeight: "500px" }}
              value={letter}
              onChange={(e) => setLetter(e.target.value)}
            />
          ) : (
            <div className="flex-1 flex items-center justify-center text-foreground/30 text-sm p-10 text-center" style={{ minHeight: "500px" }}>
