...
export default function Dashboard() {
  ...
  const stats = [
    { label: "Jobs Matched", value: jobs.length, suffix: "", color: "text-violet-700", border: "rgba(139,92,246,0.2)", glow: "rgba(139,92,246,0.1)" },
    { label: "Interviews Scheduled", value: interviews.length, suffix: "", color: "text-emerald-700", border: "rgba(52,211,153,0.2)", glow: "rgba(52,211,153,0.1)" },
    { label: "Avg Match Score", value: avgMatch, suffix: "%", color: "text-cyan-700", border: "rgba(34,211,238,0.2)", glow: "rgba(34,211,238,0.1)" },
    { label: "Live Sources", value: sources.length, suffix: "", color: "text-pink-700", border: "rgba(236,72,153,0.2)", glow: "rgba(236,72,153,0.1)" },
  ];

  if (!email) {
    return (
      <div className="flex flex-col items-center justify-center h-64 space-y-4 text-center">
        <p className="text-foreground/50">Build your CV to start scanning real jobs.</p>
  ...
  return (
    <div className="space-y-6 max-w-6xl">
      <div>
        ...
        <p className="text-foreground/40 text-sm mt-1">Live job data scanned from real sources, matched against your real CV.</p>
      </div>
        ...
        {stats.map((stat, i) => (
          <RevealItem key={i}>
            <TiltCard className="glass rounded-2xl p-5" max={8} style={{ borderColor: stat.border, boxShadow: `0 8px 32px ${stat.glow}` }}>
              <p className="text-foreground/40 text-xs mb-2">{stat.label}</p>
            ...
            <Link key={i} href={a.href} className="group block">
              <TiltCard className="glass glass-hover rounded-2xl p-4" max={10}>
                <div className="w-9 h-9 rounded-lg bg-violet-500/10 border border-violet-500/20 flex items-center justify-center mb-3 transition-colors group-hover:bg-violet-500/20">
                  <a.Icon className="w-4 h-4 text-violet-700" strokeWidth={1.75} />
                </div>
                <div className="text-foreground/80 font-semibold text-sm group-hover:text-foreground transition-colors">{a.label}</div>
                <div className="text-foreground/30 text-xs mt-0.5">{a.desc}</div>
              </TiltCard>
            </Link>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1">
          <h2 className="text-sm font-semibold text-foreground/50 uppercase tracking-widest mb-3">Scan Status</h2>
          <AgentStatus jobsAnalyzed={jobs.length} sources={sources} loading={loading} />
        </div>

        <div className="lg:col-span-2">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-semibold text-foreground/50 uppercase tracking-widest">Top Matches</h2>
            <Link href="/dashboard/jobs" className="text-xs text-violet-600 hover:text-violet-700 transition-colors">
          ...
          <RevealGroup className="space-y-3" stagger={0.08}>
              ...
              <RevealItem key={job.id}>
                <TiltCard className="glass glass-hover rounded-2xl p-4 flex items-center gap-4" max={6}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-foreground/40 text-xs truncate">{job.company} · {job.location} · {job.source}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <div className={`text-xl font-black ${job.match >= 40 ? "text-emerald-600" : job.match >= 20 ? "text-cyan-600" : "text-violet-600"}`}>
                      {job.match}%
                    </div>
            ...
            {!loading && jobs.length === 0 && (
              <p className="text-foreground/30 text-sm">No live matches yet — try adding more skills to your CV.</p>
            )}
          </RevealGroup>
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-semibold text-foreground/50 uppercase tracking-widest">Upcoming Interviews</h2>
          <Link href="/dashboard/meetings" className="text-xs text-violet-600 hover:text-violet-700 transition-colors">
            View all →
          </Link>
        </div>
        {interviews.length === 0 ? (
          <p className="text-foreground/30 text-sm">No interviews scheduled yet.</p>
        ) : (
            ...
            {interviews.slice(0, 2).map((m, i) => (
              <RevealItem key={i}>
                <TiltCard className="glass rounded-2xl p-5" max={6}>
                  <p className="text-foreground font-semibold text-sm">{m.role}</p>
                  <p className="text-foreground/40 text-xs">{m.company}</p>
