  ...
  return (
    <div className="min-h-screen text-foreground flex" style={{ background: "#ffffff" }}>
      ...
      <motion.aside
        ...
        style={{ backdropFilter: "blur(40px) saturate(180%)", background: "rgba(255,255,255,0.75)", borderRight: "1px solid rgba(20,18,35,0.07)" }}
        animate={{ x: isDesktop || sidebarOpen ? 0 : "-100%" }}
        transition={{ type: "spring", stiffness: 320, damping: 32 }}
      >
        <div className="flex items-center gap-2.5 px-6 py-5" style={{ borderBottom: "1px solid rgba(20,18,35,0.06)" }}>
          ...
          <span className="font-bold text-foreground text-base">JobFinder<span className="gradient-text">AI</span></span>
        </div>
        ...
        <div className="px-3 py-4 flex-1 overflow-y-auto">
          <div className="flex items-center gap-2 mb-4 px-3">
            ...
            <span className="text-xs text-emerald-600 font-semibold">Live job scan active</span>
          </div>
              ...
              return (
                <Link
                  ...
                  className={`relative flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-colors ${
                    isActive ? "text-violet-700" : "text-foreground/40 hover:text-foreground/80 hover:bg-foreground/5"
                  }`}
                >
                  {isActive && (
                    <motion.span
                      layoutId="sidebar-active-pill"
                      className="absolute inset-0 rounded-xl bg-violet-500/15 border border-violet-500/25 shadow-[0_0_20px_rgba(139,92,246,0.1)]"
        ...
        <div className="px-4 pb-6 pt-2 space-y-0.5" style={{ borderTop: "1px solid rgba(20,18,35,0.06)" }}>
          <Link href="/" className="flex items-center gap-2 px-3 py-2 rounded-xl text-foreground/30 hover:text-foreground/60 text-sm transition-colors">
          ...
          <button
            ...
            className="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-foreground/30 hover:text-red-600 text-sm transition-colors"
          >
      ...
      <Spotlight className="flex-1 flex flex-col min-w-0 z-10" color="124,58,237">
        <header className="px-6 py-4 flex items-center gap-4" style={{ backdropFilter: "blur(20px)", background: "rgba(255,255,255,0.6)", borderBottom: "1px solid rgba(20,18,35,0.06)" }}>
          <button className="md:hidden text-foreground/40 hover:text-foreground" onClick={() => setSidebarOpen(!sidebarOpen)} aria-label="Toggle sidebar">
            {sidebarOpen ? <X className="w-6 h-6" /> : <Menu className="w-6 h-6" />}
          </button>
          <div className="ml-auto flex items-center gap-3">
            <div className="glass rounded-full px-3 py-1.5 flex items-center gap-2">
              <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
              <span className="text-xs text-foreground/50">AI Running</span>
            </div>
