    ...
    <div className="glass rounded-2xl p-5 h-full">
      <div className="flex items-center gap-3 mb-5">
        <div className="relative">
          <div className={`w-3 h-3 rounded-full ${loading ? "bg-amber-400" : "bg-emerald-400"}`} />
      ...
      <div className="space-y-3 mb-4">
        <div className="flex items-center justify-between text-sm">
          ...
          <span className="text-emerald-400 font-bold">{loading ? "—" : jobsAnalyzed}</span>
        </div>
        <div className="flex items-center justify-between text-sm">
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-1.5">
          {sources.map((source) => (
            <span key={source} className="px-2 py-1 rounded-lg text-xs font-medium bg-violet-500/15 text-violet-300 border border-violet-500/30">
              {source}
            </span>
          ))}
        </div>
